 ...
 * @since 1.0
 */
public class RandomDataString {

	public String string(int length, boolean lowers, boolean uppers,
			boolean numbers, boolean marks) {
		char[] result = new char[length];
		StringBuilder dict = dict(lowers, uppers, numbers, marks);
		for (int i = 0; i < length; i++) {
			randomChar(dict, result, i);
		}
		return String.valueOf(result);
	}

	public String chars(boolean lowers, boolean uppers, boolean numbers,
			boolean marks) {
		StringBuilder dict = dict(lowers, uppers, numbers, marks);
		char[] result = new char[1];
		randomChar(dict, result, 0);
		return String.valueOf(result);
	}

	public String chinese(int length) {
		StringBuilder dict = new StringBuilder(chineseString);
		StringBuilder result = new StringBuilder();
		for (int i = 0; i < length; i++) {
			randomString(dict, result);
		}
		return result.toString();
	}

	private StringBuilder dict(boolean lowers, boolean uppers, boolean numbers,
			boolean marks) {
	...
	private void randomChar(StringBuilder dict, char[] result, int i) {
		int l = dict.length();
		l = (int) (l * Math.random());
		dict.getChars(l, l + 1, result, i);
	}

	private void randomString(StringBuilder dict, StringBuilder result) {
		int l = dict.length();
		l = (int) (l * Math.random());
		result.append(dict.substring(l, l + 1));
	}
	...
	private static final String numberString = "1234567890";

	private static final String markString = "!@#$%^&*()_,.?~|";

	private static final String chineseString = "一乙" + "二十丁厂七卜人入八九几儿了力乃刀又"
			+ "三于干亏士工土才寸下大丈与万上小口巾山千乞川亿个勺久凡及夕丸么广" + "亡门义之尸弓己已子卫也女飞刃习叉马乡"
			+ "丰王井开夫天无元专云扎艺木五支厅不太犬区历尤友匹车巨牙屯比互切瓦"
			+ "止少日中冈贝内水见午牛手毛气升长仁什片仆化仇币仍仅斤爪反介父从今"
			+ "凶分乏公仓月氏勿欠风丹匀乌凤勾文六方火为斗忆订计户认心尺引丑巴孔" + "队办以允予劝双书幻"
			+ "玉刊示末未击打巧正扑扒功扔去甘世古节本术可丙左厉右石布龙平灭轧东"
			+ "卡北占业旧帅归且旦目叶甲申叮电号田由史只央兄叼叫另叨叹四生失禾丘"
			+ "付仗代仙们仪白仔他斥瓜乎丛令用甩印乐句匆册犯外处冬鸟务包饥主市立"
			+ "闪兰半汁汇头汉宁穴它讨写让礼训必议讯记永司尼民出辽奶奴加召皮边发" + "孕圣对台矛纠母幼丝"
			...
			+ "并关米灯州汗污江池汤忙兴宇守宅字安讲军许论农讽设访寻那迅尽导异孙" + "阵阳收阶阴防奸如妇好她妈戏羽观欢买红纤级约纪驰巡"
			...
			+ "诊词译君灵即层尿尾迟局改张忌际陆阿陈阻附妙妖妨努忍劲鸡驱纯纱纳纲" + "驳纵纷纸纹纺驴纽"
			...
			+ "嗽蜻蜡蝇蜘赚锹锻舞稳算箩管僚鼻魄貌膜膊膀鲜疑馒裹敲豪膏遮腐瘦辣竭" + "端旗精歉熄熔漆漂漫滴演漏慢寨赛察蜜谱嫩翠熊凳骡缩"
			+ "慧撕撒趣趟撑播撞撤增聪鞋蕉蔬横槽樱橡飘醋醉震霉瞒题暴瞎影踢踏踩踪"
			+ "蝶蝴嘱墨镇靠稻黎稿稼箱箭篇僵躺僻德艘膝膛熟摩颜毅糊遵潜潮懂额慰劈"
			+ "操燕薯薪薄颠橘整融醒餐嘴蹄器赠默镜赞篮邀衡膨雕磨凝辨辩糖糕燃澡激" + "懒壁避缴"
			+ "戴擦鞠藏霜霞瞧蹈螺穗繁辫赢糟糠燥臂翼骤" + "鞭覆蹦镰翻鹰" + "警攀蹲颤瓣爆疆" + "壤耀躁嚼嚷籍魔灌" + "蠢霸露"
			+ "囊" + "罐";
}
